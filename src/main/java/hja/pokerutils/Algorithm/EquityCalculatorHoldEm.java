...
public final class EquityCalculatorHoldEm {
	...
	public static double[] calculateEquity(Config config) {
		...
		try {
			...
			for (int i = 0; i < equity.length; ++i) {
				equity[i] /= nTotal;
				config.getPlayers().get(i).setEquity(equity[i]);
			}
		}
	...
	private static int calculateBest(ArrayList<Card> combination, Config config) {
		...
		Hand bestHand = HoldEmAlgorithm.calculateHand(player.getCards(), cards);
		
		for (int i = 1; i < config.getPlayers().size(); ++i) {
			player = config.getPlayers().get(i);
			Hand hand = HoldEmAlgorithm.calculateHand(player.getCards(), cards);
